...
### ER 關聯圖（卡片式佈局）
PK/FK/REF/SRC 標籤、貝茲曲線箭頭（虛線=可空）、可隱藏 Table；卡片可拖拉移位、畫布可平移；側欄 Mermaid 原始碼可展開。FK 邊線自動識別縮寫關聯（如 `equip_id → equipments`），同列卡片弧線向右繞行避免遮擋。
...
### 全局血緣圖（Global Lineage Graph）
跨 domain / schema 的完整血緣視覺化圖。按 domain 分欄排列，節點為各資料表，彩色連線代表不同 transform 類型（direct / join / aggregate / derived / filter）。支援 domain 篩選、新增血緣邊、點擊節點查看上下游關係。自動 seed 33 條跨域血緣邊（MES Equipment、MES Process、PLM Core、Test Quality、Unified Analytics、Wip Tracking）。

![全局血緣圖](docs/screenshots/12-lineage-graph.png)

### Data Lineage 查詢（NL → SQL + 思路展示）
以自然語言詢問跨 domain 的資料關聯，AI 分 5 步驟思路串流回應（識別問題 → 搜尋 Domain → 追蹤血緣路徑 → 確認關聯鍵 → 建構 SQL），右側即時呈現當次查詢的 SVG 子圖、JOIN 路徑、涉及表格 badge 與完整 SQL。支援 mock 模式（`LINEAGE_MOCK=true`）供 LLM API 未接入時展示。

![Data Lineage 查詢](docs/screenshots/13-lineage-query.png)

### Data Lineage 查詢結果（思路 + SVG + SQL）
查詢完成後同時呈現 AI 思路過程（逐步展開）、關聯子圖高亮（金色）、JOIN 路徑文字說明與可複製 SQL。

![查詢結果](docs/screenshots/14-lineage-query-result.png)

### Schema 編輯器 — 分割面板拖拉
左側表格列表與右側欄位編輯器之間的分隔線可拖拉調整寬度，讓大型 Schema 的操作更靈活。

![Schema 編輯器分割面板](docs/screenshots/15-schema-editor-split.png)
...
| **3 層資料夾結構** | Suite → Domain（領域）→ LayerType（分層）三層自動分組，側欄清楚呈現層級 | 側邊欄 |
| **Product Suite** | 將多個 Schema 歸入同一產品套件（彩色標記），側欄可按 Suite 篩選 | 側邊欄 / Schema 設定 |
...
| **字典簽核流程** | 新增詞彙預設進入暫存區（pending），指派審核人（admin / suite_owner），審核通過後才進入正式字典；可隨時拒絕並附原因 | 命名字典 → 待審核 |
| **審核人指派** | 從具備審核權限的使用者中選擇，指派後可在待審核頁追蹤每位審核人的簽核狀態（已簽 ✓ / 未簽 ○）| 命名字典 → 待審核 |
| **使用者與角色** | 全局 4 級角色（admin / suite_owner / maintainer / viewer），權限矩陣可視化；使用者清單 CRUD；suite_owner 可指定負責的 Suite | 設定 → 使用者 & 角色 |
| **ER 圖（卡片式）** | 卡片佈局 ER 圖：PK/FK 高亮、貝茲曲線箭頭（虛線=可空、實線=非空）、可隱藏 Table；卡片可拖拉、畫布可平移、FK 邊線全程連接（含縮寫 FK）；附 Mermaid 原始碼 | ER 圖 |
| **寬表建構** | 跨 Schema 多表 JOIN 定義 + SQL VIEW 產生 + 關聯圖；R2U 與 Unified 兩個分層各自獨立 tab 維護 | 寬表 |
...
| **Data Lineage（血緣追蹤）** | 跨 domain 資料流血緣記錄，支援手動建立、寬表自動記錄、Governance publish 自動記錄 | Data Lineage |
| **全局血緣圖** | 所有 schema / table 血緣視覺化，domain 分欄，彩色連線（direct / join / aggregate / derived / filter），可篩選 domain、新增邊、點擊節點查上下游 | 全局血緣圖 |
| **NL 血緣查詢** | 自然語言跨 domain 查詢 → AI 5 步思路串流（SSE）→ 動態 SVG 子圖 + JOIN 路徑 + SQL；mock 模式供無 LLM 時展示 | Data Lineage |
| **Schema 編輯器拖拉面板** | 表格列表與欄位編輯器之間分隔線可拖拉調整寬度 | Schema 編輯器 |

---

## 使用者與角色

角色設定為全局，儲存於 `data/_sys/users.json`（隨 Git 追蹤，可作為團隊預設配置）。

| 角色 | 標識 | 審核字典 | 指派審核人 | 新增詞彙 | 刪除詞彙 | 管理使用者 |
|------|------|---------|-----------|---------|---------|-----------|
| 最高管理員 | `admin` | ✓ | ✓ | ✓ | ✓ | ✓ |
| Suite Owner | `suite_owner` | ✓（自己套件）| ✓ | ✓ | — | — |
| Maintainer | `maintainer` | — | — | ✓（進暫存區）| — | — |
| 檢視者 | `viewer` | — | — | — | — | — |

角色定義（`packages/core/src/roles.ts`）為 code-level 靜態配置；使用者-角色指派透過設定面板 UI 管理。

Suite Owner 可設定負責的 Product Suite，僅能審核該 Suite 範圍內的命名詞彙（UI 層控制，API 不做 Suite 隔離）。
...
maintainer 新增詞彙
   ...
   待審核區（Staging）
       │
       ├─ admin/suite_owner 指派審核人
       │
       ├─ 各審核人逐一簽核（signedAt 記錄時間戳）
       │
       ├─ approve → status: "approved" → 進入正式字典
       └─ reject  → status: "rejected" → 留在暫存區等修改
```

- 命名字典列表（`GET /naming-dictionary`）預設只回傳 `approved` 詞條，以確保規則引擎與命名檢查不受未審核詞條影響
- 審核人的 `signedAt` 簽核時間在指派更新時保留（重新指派同一使用者不清除已簽核記錄）
- 拒絕的詞條可由原提交者修改後重新送審
...
│   ├── _sys/                         # 系統檔（自動管理）
│   │   ├── counters.json             # 自增 ID 計數器（schema/table/field/suite/…）
...
│   ├── suites/{id}.json              # Product Suite（名稱、顏色）
│   ├── naming/{stdName}.json         # 命名字典詞條
│   ├── rules/overrides.json          # 規則覆蓋設定
│   ├── settings/domains.json         # 領域定義（id / name / color / order）
...
data/schemas/plm-core/               ← Schema slug（由名稱自動生成）
    meta.json                        ← { id, name, description, domain, suiteId, layerType, environment, targetDb, tags, createdAt, updatedAt }
    tables/
...
data/naming/
    lot_id.json                      ← { id, concept, stdName, aliases, domain, tags, status, reviewers: [...] }

data/_sys/
    users.json                       ← AppUser[]（使用者與角色清單，隨 Git 追蹤作為初始設定）
...
data/suites/
    1.json                           ← ProductSuite（{ id, name, color, description, ... }）

data/settings/
    layers.json                      ← 分層顯示名稱（{ schemaLayers, dictLayers }）
    domains.json                     ← 領域定義（{ id, name, color, order }[]）
...
- `data/_sys/counters.json`：每個實體類型（schema、table、field、naming、wideTable、suite）獨立計數器
- `data/_sys/index.json`：ID → slug 路徑的反向查找，避免全目錄掃描
- `data/_sys/users.json`：使用者與角色設定（隨 Git 追蹤，作為團隊預設設定）
...
### Data Lineage 查詢 Mock 模式

LLM API 尚未接入時，可使用 mock 模式讓 Data Lineage 查詢頁面正常展示：

```bash
# apps/api/.env.local
LINEAGE_MOCK=true   # 使用預寫的 sample 回應，不呼叫 LLM
```
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/api/v1/naming-dictionary` | 字典列表（`?domain=semiconductor&status=approved\|pending\|rejected`）|
| POST | `/api/v1/naming-dictionary` | 新增詞條（status 預設 `pending`）|
...
| POST | `/api/v1/naming-dictionary/:id/approve` | 核准詞條（status → `approved`）|
| POST | `/api/v1/naming-dictionary/:id/reject` | 拒絕詞條（status → `rejected`）|
| POST | `/api/v1/naming-dictionary/:id/reviewers` | 指派審核人 `{ reviewers: [{userId, name}] }` |

### 使用者與角色

| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/api/v1/users` | 使用者清單 |
| GET | `/api/v1/users/roles` | 角色定義（靜態）|
| POST | `/api/v1/users` | 新增使用者 |
| PATCH | `/api/v1/users/:id` | 更新使用者資料或角色 |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/api/v1/lineage` | 取得所有血緣邊（edge）|
| POST | `/api/v1/lineage` | 新增血緣邊（手動建立）|
| DELETE | `/api/v1/lineage/:id` | 刪除血緣邊 |
| POST | `/api/v1/lineage/query` | NL 查詢（非串流，回傳 JSON）|
| POST | `/api/v1/lineage/query-stream` | NL 查詢（SSE 串流，含思路步驟）|

血緣邊資料欄位：`fromSchemaId / fromTableName / fromKind / toSchemaId / toTableName / toKind / transformType / description / source`

`transformType` 可選值：`direct | join | aggregate | derived | filter`

`source` 可選值：`manual | wide-table | governance | field`
